...
@patch("pandas.read_csv")
def test_get_operations_csv_error(mock_read_csv: MagicMock) -> None:
    """
    Проверка работы функции get_operations_csv,
    которая считывает финансовые операции из CSV
    ловим ошибки
...
@patch("pandas.read_excel")
def test_get_operations_xlsx_error(mock_read_xlsx: MagicMock) -> None:
    """
    Проверка работы функции get_operations_xlsx,
    которая считывает финансовые операции из XLSX
    ловим ошибки
    ...
    :return: список словарей с транзакциями
    """
    mock_read_xlsx.side_effect = Exception
    result = extractors.get_operations_xlsx("test.csv")
    assert result == []
